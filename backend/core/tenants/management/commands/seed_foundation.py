...
class Command(BaseCommand):
    ...
    def seed_tenant_data(self, org_slug=None):
        ...
        for org in orgs:
            ...
            # 4. Seed Membership Packages (New Subscription Tiers)
            membership_packages = self.seed_membership_packages(org, features)
            
            # 5. Seed Memberships and Orders for existing clients
            self.seed_client_memberships_and_orders(org, membership_packages)
        ...
        packages_data = [
            {
                ...
                'plans': [
                    {'name': 'Monthly Silver', 'price': 1500, 'cycle': PackagePlan.BillingCycleChoices.MONTHLY},
                    {'name': 'Annual Silver', 'price': 15000, 'cycle': PackagePlan.BillingCycleChoices.YEARLY},
                ]
            },
            {
                ...
                'plans': [
                    {'name': 'Monthly Gold', 'price': 3000, 'cycle': PackagePlan.BillingCycleChoices.MONTHLY},
                    {'name': 'Annual Gold', 'price': 30000, 'cycle': PackagePlan.BillingCycleChoices.YEARLY},
                ]
            },
            {
                ...
                'plans': [
                    {'name': 'Monthly Platinum', 'price': 6000, 'cycle': PackagePlan.BillingCycleChoices.MONTHLY},
                    {'name': 'Annual Platinum', 'price': 60000, 'cycle': PackagePlan.BillingCycleChoices.YEARLY},
                ]
            }
        ]
    ...
    def seed_membership_packages(self, org, features):
        """Seeds the MembershipPackage tiers and links features."""
        tiers_data = [
            {
                'name': 'Silver 3-Month',
                'tier': MembershipPackage.TierChoices.SILVER,
                'days': 90,
                'price': 4000,
                'freezes': 5,
                'feature_codes': ['workout_plans']
            },
            {
                'name': 'Gold 6-Month',
                'tier': MembershipPackage.TierChoices.GOLD,
                'days': 180,
                'price': 15000,
                'freezes': 15,
                'feature_codes': ['workout_plans', 'diet_plans']
            },
            {
                'name': 'Platinum Annual',
                'tier': MembershipPackage.TierChoices.PLATINUM,
                'days': 365,
                'price': 45000,
                'freezes': 30,
                'feature_codes': ['workout_plans', 'diet_plans', 'personal_trainer', 'advanced_analytics', 'premium_support']
            }
        ]
        ...
        for tier_info in tiers_data:
            pkg, _ = MembershipPackage.objects.get_or_create(
                ...
                defaults={
                    ...
                    'max_freezes': tier_info['freezes'],
                    'description': f"Default {tier_info['tier']} tier package"
                }
            )
            
            # Link features
            for f_code in tier_info['feature_codes']:
        ...
        self.stdout.write(f"  - Seeded Membership Package Tiers with restricted features.")
    ...
    def seed_client_memberships_and_orders(self, org, membership_packages):
        ...
        for m in existing_memberships_no_order:
            order = Order.objects.create(
                ...
                subtotal=m.package.price,
                total_amount=m.package.price,
                payment_method='card',
        ...
        for client in clients:
            ...
            pkg = random.choice(membership_packages)
            start_date = date.today() - timedelta(days=random.randint(0, 30))
            end_date = start_date + timedelta(days=pkg.duration_days)
