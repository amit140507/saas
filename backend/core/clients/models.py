...
class Client(TenantAwareModel, BaseProfile):
    ...
    org_client = models.OneToOneField(
        OrganizationMember, on_delete=models.CASCADE, related_name='client_profile')
    ...
    status = models.CharField(
        max_length=20, choices=StatusChoices.choices, default=StatusChoices.LEAD)
    ...
    goal = models.CharField(
        max_length=20, choices=GoalChoices.choices, null=True, blank=True)
        ...
        if self.org_client_id and self.tenant_id:
            if self.org_client.tenant_id != self.tenant_id:
                raise ValidationError(
                    {"org_client": "Member must belong to the same tenant."})

        if self.assigned_trainer_id and self.tenant_id:
            if getattr(self.assigned_trainer, 'tenant_id', None) != self.tenant_id:
                raise ValidationError(
                    {"assigned_trainer": "Assigned trainer must belong to the same tenant."})
