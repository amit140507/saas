...
export default function PackagesPage() {
    ...
    return (
        <PageShell>
            <PageHeader
                title="Packages"
                description="Manage membership tiers and package availability."
                icon={PackageIcon}
                actions={(
                <Button
                    type="button"
                                ...
                                filteredPackages.map((packageItem) => (
                                    <tr key={packageItem.id} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/30 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <Badge variant={packageItem.is_active ? "success" : "neutral"}>
                            ...
                            <CardContent className="space-y-4">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        <h2 className="truncate text-base font-semibold text-foreground">{packageItem.name}</h2>
                                ...
                                <div className="space-y-2 border-t border-border pt-4">
                                    <DetailRow label="Plans" value={packageItem.plans.length} />
                                ...
                                {packageItem.features.length > 0 && (
                                    <div className="flex flex-wrap gap-1.5 border-t border-border pt-3">
                                        {packageItem.features.slice(0, 4).map((feature) => (
                                            <Badge key={feature.id} variant="neutral">{feature.feature_details.name}</Badge>
                                        ))}
                                    </div>
                                )}
                                <div className="flex justify-end gap-2 border-t border-border pt-3">
                                    <Button variant="ghost" size="icon" onClick={() => openEditModal(packageItem)} title="Edit package">
                                        <EditIcon className="h-4 w-4" />
                                    </Button>
                                    <Button variant="ghost" size="icon" onClick={() => setPackageToDelete(packageItem)} title="Delete package" className="hover:text-destructive">
